...
script_description("Скрипт предназначен для помощи игрокам в людных местах, где чат очень быстро летит и сложно общаться с покупателем/продавцом.")
...
function ev.onServerMessage(id,text ,color)
    ...
    if onTrade then
        if not #tradePlayer == 2 then return end
        result, id = sampGetPlayerIdByCharHandle(PLAYER_PED)
        if not result then return sampAddChatMessage(tag .. 'Вы не авторизованы на сервере!', colour) end
        if string.find(text, tradePlayer[1]:match("([%a_]+)")) or string.find(text, sampGetPlayerNickname(id):match("([%a_]+)")) then 
